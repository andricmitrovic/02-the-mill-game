...
void GameAI::playMovingMoveAI(MyGraphicsScene *scene) {
    ...
    std::string msg =
            "AI moved piece from " + std::to_string(std::get<1>(retVal)) + " to " + std::to_string(std::get<2>(retVal));
    Game::setGameMessage(msg);
...
void GameAI::playMillAI(MyGraphicsScene *scene) {
    ...
    std::string msg = "AI removed piece from " + std::to_string(move);
    Game::setGameMessage(Game::getGameMessage().toStdString() + "\n" + msg);
...
void GameAI::makePlayMoveAI(Player *player, int moveFrom, int moveTo) {
    ...
    Game::getGameMap()->getBoardFields()[moveTo].occupy(
...
void GameAI::revertPlayMoveAI(Player *player, int moveTo, int moveFrom) {
    Game::getGameMap()->getBoardFields()[moveFrom].deoccupy();
    Game::getGameMap()->getBoardFields()[moveTo].occupy(
            player->id() == FIELDSTATE::PLAYER_1 ? FIELDSTATE::PLAYER_1 : FIELDSTATE::PLAYER_2);
...
int GameAI::heuristicSetup() {
    ...
    std::vector<int> intersections{4, 10, 13, 19};
    ...
    for (int index = 0; index < NUM_OF_FIELDS; index++) {
        ...
        if (Game::getGameMap()->getBoardFields()[index].getPlayerID() == player) {
            ...
            if (Game::getGameMap()->getBoardFields()[checkIndex1].getPlayerID() == FIELDSTATE::EMPTY &&
                Game::getGameMap()->getBoardFields()[checkIndex2].getPlayerID() == FIELDSTATE::EMPTY) {
                if (find(std::begin(intersections), std::end(intersections), index) != std::end(intersections))
                    reward += 10;
            ...
            if (Game::getGameMap()->getBoardFields()[checkIndex3].getPlayerID() == FIELDSTATE::EMPTY &&
                Game::getGameMap()->getBoardFields()[checkIndex4].getPlayerID() == FIELDSTATE::EMPTY) {
                if (find(std::begin(intersections), std::end(intersections), index) != std::end(intersections))
                    reward += 10;
                else if (find(std::begin(sides), std::end(sides), index) != std::end(sides))
                    reward += 5;
            ...
            if ((Game::getGameMap()->getBoardFields()[checkIndex1].getPlayerID() == FIELDSTATE::EMPTY &&
                 Game::getGameMap()->getBoardFields()[checkIndex2].getPlayerID() == player) || \
                (Game::getGameMap()->getBoardFields()[checkIndex1].getPlayerID() == player &&
                 Game::getGameMap()->getBoardFields()[checkIndex2].getPlayerID() == FIELDSTATE::EMPTY)) {
                if (find(std::begin(intersections), std::end(intersections), index) != std::end(intersections))
                    reward += 100;
                else if (find(std::begin(sides), std::end(sides), index) != std::end(sides))
            ...
            if ((Game::getGameMap()->getBoardFields()[checkIndex3].getPlayerID() == FIELDSTATE::EMPTY &&
                 Game::getGameMap()->getBoardFields()[checkIndex4].getPlayerID() == player) || \
                (Game::getGameMap()->getBoardFields()[checkIndex3].getPlayerID() == player &&
                 Game::getGameMap()->getBoardFields()[checkIndex4].getPlayerID() == FIELDSTATE::EMPTY)) {
                if (find(std::begin(intersections), std::end(intersections), index) != std::end(intersections))
                    reward += 100;
                else if (find(std::begin(sides), std::end(sides), index) != std::end(sides))
