...
Game::Game(HumanPlayer & p1, HumanPlayer & p2): gameMap(new GameMap()), m_p1(p1), m_p2(p2), gameState(GAMESTATE::INIT), winner(FIELDSTATE::EMPTY), mill_occured(false) {}
...
bool Game::makePlayMove_graphical(HumanPlayer & player, unsigned moveFrom, unsigned moveTo) {
    ...
    if (isValidToSelect(moveFrom, player)) {
        ...
        } else if (gameMap -> getBoardFields()[moveTo].isOccupied()) {
